...
if not CFG_PATH.exists():
    raise FileNotFoundError(
        "Missing xpander_config.json, please run xpander init first")
...
xpander_backend: AgnoAdapter = asyncio.run(
    asyncio.to_thread(
        AgnoAdapter, agent_id=xpander_cfg["agent_id"], api_key=xpander_cfg["api_key"])
)

agno_agent_with_backend = myAgent(xpander_backend)
...
async def on_execution_request(execution_task: AgentExecution) -> AgentExecutionResult:
    ...
    try:
        ...
        xpander_backend.agent.report_execution_metrics(llm_tokens=Tokens(
            worker=llm_tokens), ai_model=agent_response.model, source_node_type="agno")
